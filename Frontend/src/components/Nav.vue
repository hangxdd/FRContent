<template>
    <Disclosure as="nav" class="bg-gray-100 shadow-md" v-slot="{ open }">
                    ...
                    <div class="hidden sm:ml-6 sm:block">
                        <div class="flex space-x-4">
                            <router-link v-for="item in navigation" :key="item.name" :to="item.href"
                                :class="[item.current ? 'bg-blue-600 text-white shadow-md' : 'text-gray-400 hover:bg-gray-400 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium']"
                                :aria-current="item.current ? 'page' : undefined">{{ item.name }}</router-link>
                        </div>
                    </div>
                </div>
                <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                    <button v-if="useAuth.user" type="button"
                    ...
                    <Menu as="div" class="relative ml-3">
                        ...
                        <!-- Sign In Button -->
                        <router-link v-if="!useAuth.user" to="/login"
                        ...
                        <!-- Register Button -->
                        <router-link v-if="!useAuth.user" to="/register"
        ...
        <DisclosurePanel class="sm:hidden">
            <div class="space-y-1 px-2 pb-3 pt-2">
                <DisclosureButton v-for="item in navigation" :key="item.name" as="router-link" :to="item.href"
                    :class="[item.current ? 'bg-blue-600 text-white shadow-md' : 'text-gray-400 hover:bg-gray-400 hover:text-white', 'block rounded-md px-3 py-2 text-base font-medium cursor-pointer']"
